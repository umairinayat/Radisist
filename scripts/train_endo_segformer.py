"""Endoscopy polyp segmentation with SegFormer-B3 (stronger than UNet).
Reuses Kvasir-SEG pairs. Interpolates logits to mask size for loss/metrics.
"""
import argparse
...
def build_pairs():
    img = ROOT / "images"; mk = ROOT / "masks"
    return [(str(f), str(mk / f.name)) for f in sorted(img.iterdir()) if (mk / f.name).exists()]
...
def _plots(hist):
    ...
    for key, title, fn in [("dice", "Endoscopy Polyp Seg (SegFormer-B3) - Dice", "dice_curve.svg"),
                           ("iou", "Endoscopy Polyp Seg (SegFormer-B3) - IoU", "iou_curve.svg"),
                           ("loss", "Endoscopy Polyp Seg (SegFormer-B3) - Loss", "loss_curve.svg")]:
...
def main(args):
    ...
    pairs = build_pairs(); print(f"[pairs] {len(pairs)}", flush=True)
    ...
    for k in splits: print(f"[split] {k}: {len(splits[k])}", flush=True)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if v["dice"] > best_dice:
            ...
            torch.save({"model_state": model.state_dict(), "arch": "segformer_b3",
                        ...
                        "task": "binary polyp segmentation", "img_size": txs.IMG_SIZE,
    ...
    metrics = {"status": "done", "disease_id": "endoscopy", "task": "binary polyp segmentation",
               ...
               "dataset": {"source": "Kvasir-SEG (debeshjha1/kvasirseg)"},
               "training": {"epochs": args.epochs, "batch_size": args.batch_size, "loss": "BCEWithLogits + Dice",
                            "mixed_precision": True}, "n_train": len(splits["train"]),
