"""Improve breast-ultrasound SEGMENTATION on BUSI (lesion masks).

Pairs each image with its *_mask.png, trains an SMP UNet (EfficientNet-B0),
BCE+Dice, mixed precision. Leakage-free stratified split. Saves metrics,
history, and plots under models/breast_us_segmentation/.
"""
import argparse
...
ROOT = REPO / "data" / "busi" / "Dataset_BUSI_with_GT"
OUT = REPO / "models" / "breast_us_segmentation"
...
def build_pairs():
    ...
    for cls in sorted(ROOT.iterdir()):
        ...
        for f in cls.iterdir():
            if not f.is_file() or "mask" in f.stem.lower():
                continue
            mask = f.with_name(f.stem + "_mask" + f.suffix)
            if mask.exists():
                pairs.append((str(f), str(mask), cls.name))
    return pairs
...
def main(args):
    ...
    txs.PLOTS_DIR.mkdir(parents=True, exist_ok=True)
    pairs = build_pairs()
    print(f"[pairs] {len(pairs)} image+mask pairs", flush=True)
    ...
    splits = {"train": [(a, b) for a, b, _ in tr],
              "val": [(a, b) for a, b, _ in va],
              "test": [(a, b) for a, b, _ in te]}
    for k in splits:
    ...
    loaders = {
        k: DataLoader(txs.SegDataset(splits[k], train=(k == "train")),
                      ...
                      persistent_workers=(nw > 0), drop_last=(k == "train"))
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if v["dice"] > best_dice:
            ...
            torch.save({"model_state": model.state_dict(), "arch": "smp_unet",
                        ...
                        "in_channels": 3, "classes": 1, "task": "binary lesion segmentation",
    ...
    metrics = {"status": "done", "disease_id": "breast_ultrasound",
               "task": "binary lesion segmentation", "best_epoch": best_ep,
               ...
               "dataset": {"source": "BUSI (aryashah2k/breast-ultrasound-images-dataset)"},
               "training": {"epochs": args.epochs, "batch_size": args.batch_size,
                            "optimizer": "AdamW", "max_lr": args.max_lr,
                            "loss": "BCEWithLogits + Dice", "mixed_precision": True},
...
def _plots(hist):
    ...
    for key, title, fn in [("dice", "Breast US Seg (UNet/eff-b0) - Dice", "dice_curve.svg"),
                           ("iou", "Breast US Seg (UNet/eff-b0) - IoU", "iou_curve.svg"),
                           ("loss", "Breast US Seg (UNet/eff-b0) - Loss", "loss_curve.svg")]:
